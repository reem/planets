...
var GravitationalConstant = 1; // 6.674e-11;
...
var Planet = function (mass, position, velocity, radius, color) { // Fill in args later.
  // Display properties
  this.$node = $("<span class='planet'></span>");
  ...
  // Internal math stuff
  this.mass = mass === undefined ? 100 : mass; // 1000000000 : mass;
  this.position =
    position || new Vector(Math.random() * 100,
                           Math.random() * 100);
  this.velocity =
    velocity || new Vector(Math.random() * 1,
                           Math.random() * 1);
...
Planet.prototype.updateVelocity = function () {
  var gravityVectors = _.map(Planet.planets,
    _.partial(gravity, this));
...
Planet.prototype.render = function () {
  this.$node.css({
    ...
    "width": (this.position.x * ScaleFactor) - this.radius
  });
};
